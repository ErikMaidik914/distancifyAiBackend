...
def auth_login(session):
    login_url = f"{session.base_url}/auth/login"
    payload = {"username": "distancify", "password": "hackathon"}
    response = request_with_retry(session, "POST", login_url, json=payload, timeout=5)
    if response:
        data = response.json()
        session._token = data.get("token")
        session._refresh_token = data.get("refreshToken")
        if session._token:
            session.headers.update({"Authorization": "Bearer " + session._token})
            return data
