...
class Simulation:
    def __init__(self, params: SimulationParams) -> None:
        ...
        # Used to exponentially back off when no emergency is returned for "/calls/next"
        self.consecutive_no_emergency = 0
        self.max_poll_sleep = 5  # maximum seconds to sleep
    ...
    def request_with_retry(self, method: str, url: str, max_retries: int = 3, backoff: float = 0.5, **kwargs) -> Optional[requests.Response]:
        for attempt in range(max_retries):
            try:
                ...
                # For calls/next, a 404 means "end of simulation" so no retry is needed.
